...
- `input_schemas` / `output_schemas` — typed *path* arrays (not wiki links) of the form `content/schemas/<base>.schema.json`. The path is a stable identifier; cast and validate resolve it to the package source `packages/<base>-schema/src/<base>.schema.json` at build time (see `packages/build-cli/src/lib/schema-paths.ts`). Vendored schemas (no Foundry-authored package) reference their schema note via `related_notes` wiki-links instead. Cast: copied verbatim into the cast bundle's `references/schemas/`.
...
8. **`validateMoldRefs`** — every Mold's typed references resolve, per kind:
   ...
   - `input_schemas` / `output_schemas` — `content/schemas/<base>.schema.json` ref string resolves (via `schema-paths.ts`) to a JSON file under the matching `packages/<base>-schema/src/`, parses as JSON Schema Draft 07.
