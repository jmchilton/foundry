#!/usr/bin/env node
// Validate a galaxy-tool-cache summarize manifest against the embedded schema.
// Exit 0 = valid; 3 = schema-validation failure; 1 = input error.

import { readFileSync } from "node:fs";
import process from "node:process";
import { validateGalaxyToolSummary } from "../validate.js";

const args = process.argv.slice(2);
if (args.length !== 1 || args[0] === "--help" || args[0] === "-h") {
  process.stderr.write("usage: validate-galaxy-tool-summary <manifest.json>\n");
