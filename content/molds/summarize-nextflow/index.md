...
- `output_schemas` / `kind: schema` → [[summary-nextflow]] (`content/schemas/summary-nextflow.schema.json`) — copied verbatim into the cast bundle's `references/schemas/`. The cast skill validates its emitted JSON with `validate-summary-nextflow` before returning. This reference is `used_at: both` because it shapes the generated skill and remains a runtime validation contract.
- `cli_commands` — none today. Open question whether the Foundry seeds a `content/cli/nextflow/` family for `nextflow config`, `nf-core list`, `nf-test`. The cast skill may call these CLIs at runtime regardless; the question is whether the Foundry carries manpages for them.
